...
int isDivisibleBy11(struct node *root)
{
	if(root!= NULL)
	{
		...
		while(temp!=NULL)
		{
			oddSum += temp->item;
			if(temp->next != NULL)
				evenSum += temp->next->item;
			temp=temp->next!=NULL?temp->next->next:temp->next;
		}
		return oddSum == evenSum || (oddSum-evenSum)%11 == 0?2:1;
	}
	return 0;
}
...
void testcases()
{
	...
	for(i=0;i<8;i++)
	{
		...
		s[j]='\0';
		root = NULL;
		if(isDivisibleBy11(stringToLinkedList(s,root))==test[i].output)
			printf("%d. Passed..!\n",i+1);
